...
describe("totalTokens field", () => {
	...
	describe.skipIf(!process.env.ANTHROPIC_API_KEY)("Anthropic (API Key)", () => {
		it("claude-3-5-haiku - should return totalTokens equal to sum of components", {
			retry: 3,
			timeout: 60000,
		}, async () => {
	...
	describe.skipIf(!process.env.XAI_API_KEY)("xAI", () => {
		it("grok-3-fast - should return totalTokens equal to sum of components", {
			retry: 3,
			timeout: 60000,
		}, async () => {
	...
	describe.skipIf(!process.env.MINIMAX_API_KEY)("MiniMax", () => {
		it("MiniMax-M2.1 - should return totalTokens equal to sum of components", {
			retry: 3,
			timeout: 60000,
		}, async () => {
	...
	describe.skipIf(!process.env.OPENROUTER_API_KEY)("OpenRouter", () => {
		...
		it("meta-llama/llama-4-maverick - should return totalTokens equal to sum of components", {
			retry: 3,
			timeout: 60000,
		}, async () => {
